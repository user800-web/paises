...
public class PaisAdapter extends RecyclerView.Adapter<PaisAdapter.PaisViewHolder> {
    ...
    public void onBindViewHolder(PaisAdapter.PaisViewHolder holder, int position) {
        ...
        holder.txtAbreviatura.setText("Abreviatura del país: "+pais.CountryCodes.getAbreviatura());
        holder.txtTelPref.setText("Prefijo telefónico del país: "+pais.getTelPref());
        holder.txtCountryInfo.setText("Link de información del país: "+pais.getCountryInfo());

        Glide.with(Ctx)
                .load(pais.CountryCodes.getIso2())
